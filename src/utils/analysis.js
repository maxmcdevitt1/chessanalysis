...
export const generateCommentary = (currentIndex, moveHistory, pgnMoves, analyzedData) => {
    // Defensive check: ensures analyzedData is an object before accessing its properties
    if (currentIndex <= 0 || !analyzedData || typeof analyzedData !== 'object') {
        return null;
    }
    ...
    // Now these accesses are safe because analyzedData has been checked
    ...
    return {
        title: `Move ${Math.ceil(currentIndex / 2)}${currentIndex % 2 !== 0 ? '.' : '...'} ${movePlayed.san}`,
        classification,
        text,
        score: (scoreCurrent / 100).toFixed(2),
        bestMove: bestMove ? `${bestMove.substring(0, 2)}-${bestMove.substring(2, 4)}` : '...',
        color,
        arrow
    };
};
