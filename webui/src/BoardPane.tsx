...
function EvalBar({ cp, pending }: { cp: number | null; pending: boolean }) {
  ...
  return (
    <div style={{ display:'flex', gap:16, alignItems:'center' }}>
      <div style={{ width:36, height:360, border:'3px solid #444', borderRadius:10, overflow:'hidden', display:'flex', flexDirection:'column' }}>
    ...
    <div style={{ display:'grid', gridTemplateColumns:'auto 360px', gap:18, alignItems:'start' }}>
      <div>
        ...
        <div style={{ display:'flex', gap:12, flexWrap:'wrap', alignItems:'center', marginTop: 12 }}>
          ...
          {typeof engineStrength === 'number' && typeof onEngineStrengthChange === 'function' ? (
            <div style={{ display:'flex', alignItems:'center', gap:10 }}>
              <span style={{ ...labelStyle, fontWeight:600 }}>Engine strength</span>
              <input type="range" min={1} max={20} step={1}
                value={engineStrength}
                onChange={e => onEngineStrengthChange(Number(e.target.value))}
              />
              <span style={{ opacity:.85 }}>{engineStrength}</span>
            </div>
          ) : null}
        ...
        {Array.isArray(evalSeries) && evalSeries.length > 0 ? (
          <div style={{ marginTop: 12, maxWidth: boardWidth, fontSize: 20 }}>
            <button onClick={onToggleEvalGraph} style={{ fontSize: 18, padding: '6px 10px', marginBottom: 8 }} aria-label="Toggle evaluation graph">
              {showEvalGraph === false ? '▸ Show eval graph' : '▾ Hide eval graph'}
            </button>
            {(showEvalGraph !== false) && <EvalSparkline series={evalSeries} height={84} />}
          </div>
        ) : null}
      </div>
