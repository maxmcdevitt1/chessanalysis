...
export async function ensureOpeningTrieLoaded(): Promise<void> {
  await ensureLoaded();
}
...
export function bookMaskForMoves(moves: string[]): boolean[] {
  if (!trieRoot) return Array(moves.length).fill(false);
  const mask: boolean[] = [];
  let node: TrieNode | null = trieRoot;
