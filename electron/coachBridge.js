...
// Keep token budget tiny to reduce RAM churn
const TOKENS_PER_ITEM = parseInt(process.env.COACH_TOKENS_PER_ITEM || '120', 10);
// Default to ndjson streaming to avoid buffering large arrays
...
async function callModelStructured({ model, prompt, expected }) {
  const baseBody = {
    ...
    options: {
      ...
      repeat_penalty: 1.05,
      num_ctx: NUM_CTX,
      num_predict: Math.max(160, TOKENS_PER_ITEM * Math.max(1, expected || 1)),
      stop: ['\n#', '\nN='],
    },
  ...
  for (const attempt of attempts) {
    let rawText = '';
    try {
      if (attempt.desc === 'ndjson' && COACH_OUTPUT_MODE === 'ndjson') {
        const streamed = await fetchNdjsonLines(attempt.body, expected);
        if (Array.isArray(streamed) && streamed.length) {
          return streamed.slice(0, expected);
        }
      }
...
function heuristicBatchNotes(batch, summary) {
  const fmtDelta = (d) => {
  ...
  return batch.map((m, i) => {
    const idx = Number.isInteger(m?.idx) ? m.idx : (Number.isInteger(m?.ply) ? m.ply : i);
    const tagRaw = String(m?.tag || '').toLowerCase();
    const tag = tagRaw.includes('book') ? 'Book'
      : tagRaw.includes('blunder') ? 'Blunder'
      : tagRaw.includes('mistake') ? 'Mistake'
      : tagRaw.includes('inacc') ? 'Inaccuracy'
      : tagRaw.includes('best') ? 'Best'
      : 'Move';
    const side = String(m?.color || '').startsWith('w') ? 'White' : 'Black';
    const delta = Number.isFinite(m?.deltaCp) ? m.deltaCp : null;
    const best = m?.bestUci && m.bestUci !== '-' ? ` Best: ${m.bestUci}.` : '';
    let mood = 'Solid idea.';
    if (tag === 'Book') mood = 'Book line; keep developing.';
    else if (tag === 'Best') mood = 'Strong choice; matches engine.';
    else if (tag === 'Inaccuracy') mood = 'Small slip; look for more active plans.';
    else if (tag === 'Mistake') mood = 'Mistake; safer continuation available.';
    else if (tag === 'Blunder') mood = 'Big miss; re-evaluate tactics.';
    const deltaTxt = delta != null ? ` Δcp=${fmtDelta(delta)}` : '';
    return {
      moveIndex: idx,
      title: `${side} move ${idx + 1}`,
      text: `${m.san || '(?)'} — ${mood}${deltaTxt}${best} [fallback]`,
      type: 'move',
    };
  });
}
...
function heuristicNotes(inputs = {}) {
  const moments = Array.isArray(inputs?.moments) ? inputs.moments : [];
  const summary = inputs?.summary || {};
  const fmtDelta = (d) => {
    if (!Number.isFinite(d)) return '';
    const v = Math.round(d);
    return v === 0 ? 'even' : (v > 0 ? `+${v}` : `${v}`);
  };
  const intro = {
    type: 'intro',
    text: `Opening: ${summary?.opening || 'Unknown'} | Acc W ${summary?.whiteAcc ?? '-'} / B ${summary?.blackAcc ?? '-'}`,
  };
  const moves = moments.map((m, i) => {
    const idx = Number.isInteger(m?.index) ? m.index : i;
    const tag = String(m?.tag || '').toLowerCase();
    const side = String(m?.side || m?.color || (idx % 2 === 0 ? 'W' : 'B')).toUpperCase().startsWith('W') ? 'White' : 'Black';
    ...
    const delta = (cpBefore != null && cpAfter != null) ? (cpAfter - cpBefore) : null;
    let mood = 'Solid move.';
    if (tag.includes('book')) mood = 'Theory line — keep developing.';
    else if (tag.includes('best')) mood = 'Strong choice; matches engine line.';
    else if (tag.includes('blunder')) mood = 'Big miss — reconsider this idea.';
    else if (tag.includes('mistake')) mood = 'Mistake — cleaner options available.';
    else if (tag.includes('inacc')) mood = 'Small slip — tighten the plan.';
    const cpNote = delta != null ? ` Δcp=${fmtDelta(delta)}` : '';
    const best = m?.best ? ` Best: ${m.best}.` : '';
    return {
      type: 'move',
      moveIndex: idx,
      title: `${side} move ${idx + 1}`,
      text: `${m.san || '(?)'} — ${mood}${cpNote}${best}`,
    };
  });
  const summaryNote = {
    type: 'summary',
    text: `Avg CPL W ${fmt(summary?.avgCplW)} / B ${fmt(summary?.avgCplB)}. Estimated strength W ${fmt(summary?.estEloWhite)} / B ${fmt(summary?.estEloBlack)}.`,
  };
  return [intro, ...moves, summaryNote];
}
...
async function generateNotes(inputs) {
  ...
  for (let b = 0; b < batches.length; b++) {
    ...
    const batch = batchRaw.map((m, idx) => {
      const cpBefore = Number.isFinite(m?.cpBefore) ? Number(m.cpBefore) : null;
      const cpAfter = Number.isFinite(m?.cpAfter) ? Number(m.cpAfter) : null;
      const deltaCp = cpAfter != null && cpBefore != null ? cpAfter - cpBefore : null;
      return {
    ...
    // Fallback: if the model gave too few items, pad with stubs to keep UI consistent.
    if (norm.length < expected) {
      const stubs = heuristicBatchNotes(batch, summary);
      // Fill missing slots; if none from model, use all stubs.
      const byIdx = new Map(norm.map((n) => [n.moveIndex, n]));
      for (const stub of stubs) {
        if (!byIdx.has(stub.moveIndex)) norm.push(stub);
      }
      norm.sort((a, b) => a.moveIndex - b.moveIndex);
      norm = norm.slice(0, expected);
    }
  ...
  ipcMain.handle('coach:generate', async (_e, payload) => {
    try {
      ...
      if (!available) {
        ...
        return { notes: heuristicNotes(inputs), offline: false, reason: 'model-missing', model, fallback: true };
      }
      const res = await generateNotes({ ...inputs, model });
      if (res?.notes && Array.isArray(res.notes)) return res;
      return { notes: heuristicNotes(inputs), offline: false, reason: 'empty-response', model, fallback: true };
    } catch (e) {
      console.error('[coach] error:', e && (e.stack || e.message || e));
      return { notes: heuristicNotes(payload?.inputs), offline: false, reason: 'exception', fallback: true };
    }
  });
